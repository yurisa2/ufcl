...
### Plot e teste de tendência nos dados ###
#library(Kendall)
plot(alabama$appl,type='l',ylab="Daily demand",xlab="Days")
plot(density(alabama$appl))
getwd()

jpeg("resultados/histograma.jpg", quality=300)
par(mar=c(5,5,2,2))
h <- hist(alabama$appl, main="", xlab="", ylab="", xaxt='n', yaxt='n')
axis(1,seq(0,6000,1000),seq(0,6000,1000),cex.axis=1.2)
axis(2, seq(0,60,10),seq(0,60,10),cex.axis=1.2)
mtext("Daily demand",1,line=3,cex=1.5)
mtext("Frequency",2,line=3,cex=1.5,padj=0)
dev.off()

acf(alabama$appl)
pacf(alabama$appl)
#MannKendall(alabama$appl)
#summary(MannKendall(alabama$appl))
##If the autocorrelation and/or partial autocorrelation plots of the
##time series of annual precipitation levels would have revealed the 
##presence of significant temporal dependence in the data across years,
##we could have used the Mann-Kendall test in conjunction with the block
##bootstrap method, as follows:  
#library(boot)
#MKtau <- function(z) MannKendall(z)$tau
#tsboot(PrecipGL, MKtau, R=500, l=5, sim="fixed")
#Parece que há um problema ao usar o teste MannJendall junto com 
#o block bootstrap method
### END TESTE DE TENDÊNCIA ###
...
for(mcs in 1:REPETITIONS){
  ...
  ### Step 1 - FCM ###
  al.cl <- cmeans(alabama$appl,k0, method=fcm.method)
...
setwd("resultados/")
write.csv(final, paste("contral - ",toupper(fcm.method)," fdsOUT.csv", sep=""), row.names=FALSE)
