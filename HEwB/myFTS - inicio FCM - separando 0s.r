...
#
# FCM: separando 0's e rodando FCM para k0-1 centróides
#
###############################################################################
...
PATH <- "D:\\Dropbox\\_pessoal\\My Papers\\Fuzzy Times Series"
#ORDEM n+1 para calcularmos até ordem n
ORDEM <- 15 #10    #High-order FTS. Maximum of 9th order
...
dados <- read.csv("dados\\contral.csv", header=TRUE)
...
for(mcs in 1:REPETITIONS){
  ...
  index0 <- which(alabama$appl==0)
  index.sem0 <- which(alabama$appl!=0)
  dados.sem0 <- alabama$appl[index.sem0]
  ...
  ### Step 1 - FCM ###
  al.cl <- cmeans(dados.sem0,k0-1, method=fcm.method)
  #incluir os dados iguais a 0 ANTES de ordenar
  #primeiro criarei matriz com 0s, faltando a primeira coluna para A1
  temp <- matrix(0, nrow=length(alabama$appl), ncol=k0-1)
  temp[index.sem0,] <- al.cl$membership
  temp <- cbind(rep(0,nrow(temp)),temp)
  temp[index0,1] <- 1
  al.cl$membership <- temp
  #size
  al.cl$size <- c(length(index0), al.cl$size)
  #cluster
  temp <- rep(1,length(alabama$appl))
  temp[index.sem0] <- al.cl$cluster+1
  al.cl$cluster <- temp
  #centers
  al.cl$centers <- rbind(0,al.cl$centers)
  rownames(al.cl$centers) <- 1:nrow(al.cl$centers)
  ...
  #Identifica todas as precedências - Ordem nth
  #----------
  prec <- vector("list",ORDEM)
  for(n in 1:ORDEM) assign(paste("prec",n,sep=""), c(0,fts[1:n]))     #Iniciando variáveis com NULL, prec1, prec2, prec3...
...
setwd("resultados/")
write.csv(final, paste("contral - ",toupper(fcm.method)," - separando 0s",ifelse(SEM.FINAL.DE.SEMANA," fdsOUT",""),".csv",sep=""), row.names=FALSE)
...
## Separando 0s em cada inicialização: FCM
#MAPE: 53.07%
#RMSE: 228.6
#MSE: 52245.21

## Separando 0s em cada inicialização: UFCL
#MAPE: 51.78%
#RMSE: 282.6
#MSE: 79841

## Inicializando Centroides (diferente inicio para cada repetição): FCM
#MAPE: 52.22%
#RMSE: 266.3
#MSE: 70932 

## Inicializando Centroides (diferente inicio para cada repetição): UFCL
#MAPE: 49.93%
#RMSE: 168.7
#MSE: 28458 

## Inicializando Centroides (mesmo inicio para cada repetição): FCM
#MAPE: 54.38%
#RMSE: 280.3
#MSE: 78591

## Inicializando Centroides (mesmo inicio para cada repetição): UFCL
#MAPE: 55.11%
#RMSE: 3035
#MSE: 92117
